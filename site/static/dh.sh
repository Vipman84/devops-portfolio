#!/bin/bash
# DevOps Helper (dh) - интерактивный справочник с готовыми списками
...
# ================== РАЗДЕЛ «ПРОЦЕССЫ И СЛУЖБЫ» (с готовым списком) ==================
section_processes() {
    while true; do
        clear
...
list_and_manage_services() {
    ...
    # Получаем список активных служб
...
# ================== РАЗДЕЛ «ПОЛЬЗОВАТЕЛИ И ГРУППЫ» (с готовым списком) ==================
section_users() {
    while true; do
        clear
...
list_users() {
    ...
    # Показываем обычных пользователей (UID >= 1000)
...
# ================== РАЗДЕЛ «СЕТЬ» (с готовым списком интерфейсов) ==================
section_network() {
    while true; do
        clear
        echo "--- Сеть и диагностика ---"
        echo " 1. Показать сетевые интерфейсы"
        ...
        case $net_choice in
            1) ip -br addr; read -p "Нажмите Enter..." ;;
            ...
            3) ss -tlnp
               echo ""
               echo "Пояснение:"
               echo "LISTEN - порт слушается"
               echo "ESTAB - установлено соединение"
               read -p "Нажмите Enter..." ;;
...
# ================== РАЗДЕЛ «УСТАНОВКА ПАКЕТОВ (apt)» (с готовым списком) ==================
section_apt() {
    while true; do
        clear
        echo "--- Установка пакетов (apt) ---"
        echo " 1. Обновить список пакетов (apt update)"
        echo " 2. Обновить систему (apt upgrade)"
        echo " 3. Установить популярные пакеты"
        echo " 4. Установить свой пакет"
        echo " 5. Удалить пакет"
        echo " 6. Поиск пакетов"
        echo " 0. Назад"
...
install_popular_packages() {
    ...
    echo " 1. htop          - монитор процессов"
    echo " 2. git           - система контроля версий"
    echo " 3. curl          - утилита для HTTP-запросов"
    echo " 4. vim           - текстовый редактор"
    echo " 5. build-essential - компиляторы и инструменты"
    echo " 6. python3-pip   - менеджер пакетов Python"
    echo " 7. nodejs        - JavaScript runtime"
    echo " 8. docker.io     - контейнеризация"
    echo " 0. Назад"
