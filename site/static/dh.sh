...
menu_collector() {
    while true; do
        ...
        case $c in
            ...
            3) pkill -f api_collector.py; sleep 1; cd ~/lottery-bot; cd ~/lottery-bot && ./venv/bin/python api_collector.py >> data/collector.log 2>nohup python api_collector.py >> data/collector.log 2>&1 &1 & echo "Сборщик перезапущен";;
